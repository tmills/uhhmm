...
def sampleDirichlet(counts, H):
    ...
    for ind,val in np.ndenumerate(counts.pairCounts[...,0]):
        ...
        base = np.zeros((1,L-1)) + counts.pairCounts[ind][1:]+H[1:]
...
def sampleBernoulli(counts, H):
    ...
    for ind,val in np.ndenumerate(counts.pairCounts[...,0]):
        base = counts.pairCounts[ind][:] + H.flatten()
