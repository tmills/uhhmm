...
def start_cluster_workers(work_distributer, cluster_cmd, maxLen):
    logging.debug("Cluster command is %s" % cluster_cmd)

    cmd_str = 'python3 %s/scripts/workers.py %s %d %d %d %d' % (os.getcwd(), work_distributer.host, work_distributer.jobs_port, work_distributer.results_port, work_distributer.models_port, maxLen)
...
def start_local_workers_with_distributer(work_distributer, maxLen, num_workers):
    logging.info("Starting workers with maxLen=%d and num_workers=%d" % (maxLen, num_workers) )
    return start_local_workers(work_distributer.host, work_distributer.jobs_port, work_distributer.results_port, work_distributer.models_port, maxLen, num_workers)
    
def start_local_workers(host, jobs_port, results_port, models_port, maxLen, num_workers):
    ...
    for i in range(0, num_workers):
        fs = PyzmqWorker.PyzmqWorker(host, jobs_port, results_port, models_port, maxLen, tid=i)
...
def main(args):
    ...
    if len(args) == 6:
        ...
        processes = start_local_workers(args[0], int(args[1]), int(args[2]), int(args[3]), int(args[4]), num_workers)
...
#             p.start()
        ...
        start_local_workers(args[0], int(args[1]), int(args[2]), int(args[3]), int(args[4]), 1)
